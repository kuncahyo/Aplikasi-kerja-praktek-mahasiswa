...
include '../koneksi/koneksi.php';
    ...
    $nip = mysqli_real_escape_string($koneksi, $_REQUEST['nip']);
    ...
    //       if ($row["total"] < 1){
                $sql = "update ppkp SET nip='".$nip."',nama='".$name."',email='".$email."',status='".$status."',status_akun='".$status_akun."' where id = '".$id."'";
                $result = $koneksi->query($sql);

            
                echo "<script type='text/javascript'>location.replace('../Editing-ppkp.php');</script>";
    //         }else{
