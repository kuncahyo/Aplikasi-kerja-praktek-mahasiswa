...
if (isset($_POST['approve'])) {
    $id = $_POST['id'];
    $sql2 = "UPDATE `kaprodi` SET `status_akun`='approve' WHERE id=$id";
    if(mysqli_query($koneksi, $sql2)){
        echo "<script type='text/javascript'>location.replace('../Request-Login-Kaprodi.php');</script>";
...
if (isset($_POST['decline'])){
    $id = $_POST['id'];
    $sql2 = "UPDATE `kaprodi` SET `status_akun`='decline' WHERE id=$id";
    if(mysqli_query($koneksi, $sql2)){
        echo "<script type='text/javascript'>location.replace('../Request-Login-Kaprodi.php');</script>";
        // echo $file."<br>".$kodesesi."<br>".$nama."<br>".$no;
  } else{
