...
if (isset($_POST['tambah-form-ttd'])) {
    ...
    if( move_uploaded_file($_FILES['form-ttd']['tmp_name'],$direktory.$_FILES['form-ttd']['name']) and mysqli_query($koneksi, $sql2)){
        echo "<script type='text/javascript'>location.replace('../Form-tanda-tangan-kaprodi.php');</script>";
...
if (isset($_POST['salah'])) {
    ...
    $id=$_POST['id_ttd'];
    $info=$_POST['info'];
    $sql2 = "UPDATE `tanda_tangan` SET `info`=' $info',`tanggal`='$tgl',`status`='Decline' where id=$id";
